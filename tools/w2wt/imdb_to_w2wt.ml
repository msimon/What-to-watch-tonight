...
let movie config mongo_genre =
  lwt mongo = Mongo.create config.database.ip config.database.port config.database.name "movies" in
  (* lwt mongo_genre = Mongo.create config.w2wt.ip config.w2wt.port config.w2wt.name "genres" in *)
  ...
  let rec add_movie acc r =
    ...
    if MongoReply.get_num_returned r = 0l then Lwt.return acc
      ...
      lwt movies =
        Lwt_list.fold_left_s (
          fun acc d ->
            ...
            lwt genres =
              (** mongo_lwt doesn't work on several request. Insert works, but read can get messed up **)
              Lwt_list.map_p (
                fun g ->
            ...
            Lwt.return ({
              Movie_w2wt.uid = Uid.fresh_uid Uid.Movie ;
              ...
              popularity = m.Movie_api.popularity ;
      ...
      (* let mongo = Mongo.change_collection mongo "movies" in *)
  ...
  lwt _ = Mongo.ensure_simple_index mongo "uid" in
  let movies_bson =
    List.map (
      fun m ->
        Movie_w2wt.Bson_utils_movie.to_bson m
    ) movies
  in

  lwt _ = Mongo.insert mongo movies_bson in
...
let genre config =
  ...
  Printf.printf "creating genres dbs!\n%!";
  lwt _ = Mongo.ensure_simple_index mongo "uid" in
  let genres_bson =
    List.map (
      fun g ->
...
let run () =
  ...
  Printf.printf "finish with genre starting with movies\n%!" ;
  lwt _ = movie config mongo_genre in
